# Sistema de pontuação do Agent Platform Advisor
...
- **Delegar** → duas perguntas de desdobramento decidem entre Cowork e Scout. Elas são feitas **progressivamente**: a Cadência aparece primeiro e o Alcance só é revelado depois que a cadência é respondida (assim as duas nunca aparecem ao mesmo tempo).
  ...
  | Pergunta | Opções |
  |---|---|
  | **Cadência** (perguntada primeiro): como o agente deve trabalhar? | Sob demanda (concluir um trabalho de várias etapas de uma vez, com vários artefatos ou um processo entre sistemas) · Contínua (sempre ativo, gerenciar e coordenar meu dia) · Ainda não sei |
...
|---|---|---|---|---|---|
| Usuário de negócio ou especialista, sem programação | q1a | **3** | 1 | 0 | 1 |
...
|---|---|---|---|---|---|---|
| Eu ou um pequeno time interno | q8a | **3** | 2 | 1 | 1 | nenhuma |
| Área ou público interno amplo | q8c | 1 | **3** | 2 | 2 | nenhuma |
| Usuários externos | q8b | 0 | **3** | **3** | 2 | Zera AB, M365 |
| Ainda não decidido | q8d | 2 | 2 | 1 | 1 | nenhuma |
...
| Vários lugares / não decidido | q2d | 1 | **3** | **3** | 1 | nenhuma |
...
| Opção | ID | Agent Builder | CS | Foundry | DBX | Regra rígida |
|---|---|---|---|---|---|---|
| Perguntas e respostas simples / consultas | q4a | **3** | **3** | 1 | 2 | nenhuma |
| Conversacional (múltiplos turnos) | q4b | 2 | **3** | 2 | 2 | nenhuma |
| Criar/analisar conteúdo no Copilot | q4e | **3** | 2 | 2 | 1 | nenhuma |
...
| Sites públicos ou arquivos enviados | q3e | **3** | 2 | 1 | 1 | nenhuma |
...
As preferências por perfil forçam uma plataforma acima de outra no ranking, independentemente das pontuações. Diferente das regras rígidas, todas as pontuações são preservadas e o ajuste só afeta a ordem. Uma justificativa é exibida como fator-chave no card de recomendação.
...
> O espaço de combinações cresceu de 1.920 para 4.200 quando o Agent Bricks acrescentou q1e, q2e, q4f e q3g (5 × 4 × 5 × 6 × 7). A fatia do Agent Builder caiu de 3,1% para 1,8% não porque sua pontuação mudou, mas porque as quatro novas opções o desqualificam e o denominador cresceu com combinações para as quais ele nunca foi elegível. Regere esses números com uma varredura por força bruta sobre `rankPlatforms()` sempre que a matriz mudar.
