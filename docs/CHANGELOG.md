...
- **Databricks Agent Bricks joins the scored build assessment** as a fifth platform (`databricks`) and the first non-Microsoft option in the matrix. It covers the scenario the advisor previously had no answer for: the agent's value comes from the data estate, not from Microsoft 365 — knowledge governed in Unity Catalog, built and served by the team that owns it. Sourced from the [Agent Bricks product page](https://www.databricks.com/product/artificial-intelligence/agent-bricks) and [Databricks documentation](https://docs.databricks.com/aws/en/generative-ai/agent-bricks/).
  - **The questions were extended rather than added to** — still five questions and a 0–15 scale, so shared links, thresholds, and the score UI are unchanged. Four new options give the data-platform scenario somewhere to land: **q1e** (data platform / data engineering team), **q2e** (delivery in the data and analytics environment), **q4f** (reason over large volumes of enterprise data — extraction across document collections, natural-language questions over governed tables, quality tuning on your own data), and **q3g** (lakehouse / Unity Catalog / Delta tables). All four are appended last within their question, so existing answer indices and share links keep resolving to the same options.
  - **Agent Bricks is disqualified where it genuinely cannot go**, using the same hard-rule mechanism that already enforces Agent Builder's limits: **q3a / q3b / q3d** (Microsoft 365 content and the Copilot connector catalog — Agent Bricks grounds on the lakehouse, not SharePoint, OneDrive, Teams, or Outlook) and **q2a** (Microsoft 365 Copilot chat — agents are served from Databricks with no native publishing path there). It therefore never competes for the classic Copilot scenario; it appears only when the user says the scenario is anchored in their data estate. The four new options also zero Agent Builder, which cannot reach any of them.
  - **New tiebreakers and pairs:** lakehouse data (q3g) prefers Agent Bricks over Copilot Studio on a tie; a data platform team (q1e) prefers it over Foundry. Two complementary pairs were added — *Agent Bricks + Foundry* (lakehouse reasoning plus the Azure application, identity, and Microsoft 365 publishing) and *Copilot Studio + Agent Bricks* (reason over lakehouse data, deliver the answer in Teams).
  - **New contradiction and persona notes:** lakehouse knowledge + Copilot chat delivery, business user + lakehouse, and data platform team + Microsoft 365 content; plus winner-persona warnings when Agent Bricks wins for a business user or a low-code maker, since it presumes a Databricks workspace, Unity Catalog permissions, and serving capacity.
  - Added the platform card to the start page and the *Build agents* exploration group, plus `images/databricks.svg` — a placeholder lakehouse-layers mark in the Databricks signal colour, to be swapped for the official brand asset.
  - **Explore now uses the wide workspace width.** A fourth Build card orphaned itself on its own row inside the 1024px reading container. `.exploration-grid` moved from a fixed 3 columns to `auto-fit` (which collapses the unused track, so the 3-card *Use* group still spans 3 across), and `showSection()` applies `.main-container--wide` on the explore section only — the 1200px instrument workspace width that `docs/DESIGN.md` already sanctions for comparison surfaces. Both groups now fill exactly one row at ≥1100px; prose-heavy wizard panels keep the 1024px reading width.
  - Regenerated the distribution analysis in `docs/SCORING.md` by brute-forcing all combinations through the live `rankPlatforms()`: the space grew from 1,920 to 4,200 combos (5 × 4 × 5 × 6 × 7). Copilot Studio 68.7%, Foundry 20.3%, Agent Bricks 9.3%, Agent Builder 1.8%. Agent Builder's share fell from 3.1% only because the four new options all disqualify it — its scoring is unchanged.
  - Dropped "Microsoft" from the start-page lead and the `description` / `og:description` meta tags ("recommend the best **Microsoft** platform for your agent needs" → "the best platform"), since the advisor can now land on a non-Microsoft platform. Known follow-up: `images/og-image.png` is a capture of the start page from when it showed six tiles, and its `og:image:alt` describes six — both need a refresh.
  - New `tests/e2e/databricks-path.spec.js` covers the welcome card, the exploration group, a strong-fit lakehouse win, both disqualification paths, and clicking through the new wizard options (45 tests passing).
