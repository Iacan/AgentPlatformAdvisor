...
| Build | Databricks Agent Bricks | Agents built, evaluated, governed, and served on the lakehouse, grounded in Unity Catalog tables and governed document collections |
...
2. **Custom agent assessment** — runs the scored 5-question wizard for Agent Builder, Copilot Studio, Foundry, and Databricks Agent Bricks.
...
- Who is building the agent: business user, low-code maker, professional developer, data scientist/AI engineer, or data platform team
- Who will use it: small internal team, broad internal audience, external users, or undecided
- Where users will interact with it: Microsoft 365 Copilot chat, custom app, background/event-triggered runtime, the data and analytics environment, or multiple places
- What the agent should do: Q&A, multi-turn conversation, content/data analysis, multi-step action workflows, complex orchestration, or reasoning over large volumes of enterprise data
- What information it needs: Microsoft 365 content, connector-backed systems, Dataverse/custom APIs, public web/uploaded files, custom retrieval architecture, or lakehouse data governed in Unity Catalog
...
2. **Custom agent assessment** is scored across Agent Builder, Copilot Studio, Foundry, and Databricks Agent Bricks:
...
- **Databricks Agent Bricks** is the non-Microsoft option, included for scenarios anchored in the data platform rather than in Microsoft 365: agents built, evaluated, governed, and served next to lakehouse data, under the same Unity Catalog permissions and lineage that already cover that data. It is disqualified when the knowledge lives in Microsoft 365 or the agent must run inside Microsoft 365 Copilot chat.
